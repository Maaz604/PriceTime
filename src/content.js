...
// Detects thumbnail picker cards: narrow containers that also contain an image.
// This pattern covers Amazon's variant selector, eBay image+price tiles, etc.
// Requiring an <img> prevents false positives on sites like BestBuy where
// legitimate price containers can be narrow.
  ...
  const walker = document.createTreeWalker(
    ...
    NodeFilter.SHOW_TEXT,
    {
      acceptNode(node) {
        ...
        // Skip text nodes inside visually hidden elements (e.g. .a-offscreen on Amazon).
        // These have real text content but are positioned off-screen, so injecting
        // there would place the label somewhere the user cannot see.
...
// Some sites (notably Amazon) split a single price across several inline elements:
//   <span>$</span><span>43</span><span class="cents">49</span>
// No individual text node matches the full price pattern, so the text node scanner
// finds nothing. This function looks for elements whose assembled textContent forms
// a price but whose child text nodes don't individually match.
function scanSplitPriceElements(root = document.body) {
  ...
  for (const el of candidates) {
    ...
    // If any child text node already contains a full price, the text node scanner
    // handles this element — skip it to avoid double injection.
