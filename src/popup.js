...
function syncBodyState(enabled) {
  if (enabled) {
    mainBody.classList.remove('popup__body--disabled');
  } else {
    mainBody.classList.add('popup__body--disabled');
  }
}
...
saveBtn.addEventListener('click', () => {
  ...
  if (isNaN(wage) || wage <= 0) {
    wageInput.focus();
    showStatus('Enter a valid hourly wage.');
    return;
  }

  if (isNaN(threshold) || threshold < 1) {
    thresholdInput.focus();
    showStatus('Enter a valid threshold.');
    return;
  }

  chrome.storage.sync.set(
    { hourlyWage: wage, highlightThreshold: threshold },
    () => {
